...
export default function MapCanvas() {
  ...
  const showGenerated = localStorage.getItem('show_generated_pins') === 'true';
  const allPins = useMemo(() => {
    const combined = [...samplePins, ...dbPosts];
    if (showGenerated) return combined;
    // Only show pins backed by a real organization (sourceOrg) or user-submitted DB posts
    return combined.filter(p => !!p.sourceOrg || !!(p as any).profileId);
  }, [samplePins, dbPosts, showGenerated]);
